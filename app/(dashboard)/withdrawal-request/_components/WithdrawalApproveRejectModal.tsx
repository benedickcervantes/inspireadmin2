...
interface WithdrawalApproveRejectModalProps {
  open: boolean;
  onClose: () => void;
...
export default function WithdrawalApproveRejectModal({
  open,
  onClose,
  onConfirm,
  action,
  referenceId,
  amount,
}: WithdrawalApproveRejectModalProps) {
  ...
  const title = isApprove ? "Approve Withdrawal Request" : "Reject Withdrawal Request";
    ...
    try {
      ...
      toaster.push(
        <Message showIcon type="success">
          {isApprove ? "Withdrawal request approved successfully" : "Withdrawal request rejected"}
        </Message>,
  ...
  return (
    <Modal open={open} onClose={handleClose} size="xs" backdrop="static" className="dark-modal">
      <Modal.Header>
        <div className="flex items-center gap-3">
          <motion.div
            className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              isApprove ? "bg-[var(--success-soft)] border border-[var(--success)]/20" : "bg-[var(--danger-soft)] border border-[var(--danger)]/20"
            }`}
            initial={{ scale: 0, rotate: -180 }}
            animate={{ scale: 1, rotate: 0 }}
            transition={{ type: "spring", stiffness: 200 }}
          >
            {isApprove ? <Icons.Check className="w-5 h-5 text-[var(--success)]" /> : <Icons.X className="w-5 h-5 text-[var(--danger)]" />}
          </motion.div>
          <div>
            <Modal.Title className="text-lg font-semibold text-[var(--text-primary)]">{title}</Modal.Title>
            <p className="text-xs text-[var(--text-muted)] mt-0.5">
              {referenceId && <span className="font-mono">{referenceId}</span>}
              {amount != null && <span className="ml-1">₱{amount.toLocaleString()}</span>}
            </p>
          </div>
        </div>
      </Modal.Header>
      <Modal.Body className="!bg-[var(--surface)] !px-6 !py-6">
        ...
        <Form fluid>
          ...
          <Form.Group>
            <Form.ControlLabel className="text-sm font-medium text-[var(--text-secondary)]">Notes (optional)</Form.ControlLabel>
            <Input
              as="textarea"
              rows={3}
              value={notes}
            ...
            />
            {notes.length > 0 && <p className="text-[10px] text-[var(--text-muted)] mt-0.5">{notes.length}/500</p>}
          </Form.Group>
          {error && (
            <motion.div
      ...
      <Modal.Footer className="!bg-[var(--surface)] !border-t !border-[var(--border)] !pt-4 !pb-4">
        <div className="flex gap-3 justify-end mx-4">
          <Button onClick={handleClose} appearance="subtle" disabled={isLoading}>Cancel</Button>
          <Button
